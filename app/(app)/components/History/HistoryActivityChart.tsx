  ...
  if (active && payload && payload.length) {
    return (
      <div className="p-4 bg-white/90 backdrop-blur-sm border border-gray-200 rounded-lg shadow-lg">
        ...
        <p className="intro" style={{ color: '#1f2937' }}>{`Workouts : ${payload[0].value}`}</p>
      ...
      <div style={{ width: '100%', height: 300 }}>
        <ResponsiveContainer>
          <ComposedChart data={data} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
            ...
            <Bar dataKey="workouts" fill="#1f2937" name="Workouts" radius={[10, 10, 0, 0]} barSize={20} />
            <Line type="monotone" dataKey="goal" stroke="#fbbf24" strokeWidth={2} name="Goal" strokeDasharray="5 5" dot={false} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
    </div>
