...
export default function NutritionResults({ userData, onEdit }: NutritionResultsProps) {
  ...
  const { data: summary, mutate: mutateSummary } = useSWR('/api/nutrition/summary', fetcher);
  const { data: foodLogsData, mutate: mutateLogs } = useSWR('/api/nutrition/log', fetcher);
  const { data: waterData, mutate: mutateWater } = useSWR('/api/nutrition/water', fetcher);
  const { data: recommendedFoodsData } = useSWR('/api/nutrition/recommendations', fetcher);
  const { data: dashData } = useSWR('/api/stats/dashboard', fetcher);

  const [waterIntake, setWaterIntake] = useState(0);
  const [activityType, setActivityType] = useState<'rest' | 'normal' | 'heavy'>('normal');

  const foodLogs = foodLogsData || [];
  const recommendedFoods = recommendedFoodsData || [];

  // Sync internal state with SWR data
  useEffect(() => {
    if (waterData) setWaterIntake(waterData.amountMl || 0);
  }, [waterData]);
  ...
  const refreshData = useCallback(() => {
    ...
    mutateLogs();
    mutateWater();
  }, [mutateSummary, mutateLogs, mutateWater]);
  ...
  const handleAddWater = async (amount: number) => {
    ...
    mutateWater({ amountMl: newAmount }, false);
    try {
    ...
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden border-none shadow-2xl bg-white dark:bg-neutral-950 relative group">
        ...
        <CardContent className="p-6 sm:p-8 relative">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
            ...
            <Button onClick={onEdit} variant="outline" size="sm" className="rounded-full h-10 px-5 border-neutral-200 dark:border-neutral-800 hover:bg-neutral-50 dark:hover:bg-neutral-900 font-bold text-xs transition-all active:scale-95 shadow-sm">
             ...
             <div className="flex-grow w-full space-y-8">
                <div className="grid grid-cols-1 gap-5">
                   ...
                   <div className="relative pt-2">
                     <Progress value={calPercentage} className="h-4 bg-neutral-100 dark:bg-neutral-900 rounded-full border border-neutral-200/20 dark:border-neutral-800/20" indicatorClassName="bg-gradient-to-r from-emerald-400 to-emerald-600 rounded-full shadow-[0_0_15px_rgba(16,185,129,0.3)]" />
                     {calPercentage > 0 && <div className="absolute top-[18px] h-4 w-1 bg-white/40 blur-[1px] rounded-full transition-all duration-1000" style={{ left: `${Math.min(98, calPercentage)}%` }} />}
                   </div>
                </div>
                <div className="grid grid-cols-3 gap-3 sm:gap-4">
                   <MacroRing label="Protein" consumed={summary?.consumed?.protein || 0} target={targetProtein} color="emerald" icon={Zap} />
                   <MacroRing label="Carbs" consumed={summary?.consumed?.carbs || 0} target={targetCarbs} color="blue" icon={Target} />
                   <MacroRing label="Fat" consumed={summary?.consumed?.fat || 0} target={targetFat} color="orange" icon={Flame} />
                </div>
             </div>
          </div>
        </CardContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="md:col-span-2 shadow-sm border-none bg-white dark:bg-neutral-900 rounded-3xl overflow-hidden">
          <CardHeader className="flex flex-row items-center justify-between pb-4 border-b border-neutral-50 dark:border-neutral-800">
            <CardTitle className="text-xl font-bold flex items-center gap-3">
              <div className="p-2 rounded-2xl bg-orange-500/10 text-orange-600"><Utensils className="w-5 h-5" /></div>
              Today's Meals
            </CardTitle>
            <Button size="sm" onClick={() => setIsAddFoodOpen(true)} className="rounded-full bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 hover:opacity-90 transition-opacity">
          ...
          <CardContent className="pt-6">
            {foodLogs.length > 0 ? (
                  ...
                  <div key={log.id} className="group flex items-center justify-between p-4 rounded-2xl bg-neutral-50 dark:bg-neutral-800/50 hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-all duration-300 border border-transparent hover:border-neutral-200 dark:hover:border-neutral-700">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl bg-white dark:bg-neutral-900 flex items-center justify-center shadow-sm text-lg">
                        {log.foodName.toLowerCase().includes('chicken') ? '🍗' : log.foodName.toLowerCase().includes('egg') ? '🥚' : log.foodName.toLowerCase().includes('milk') ? '🥛' : log.foodName.toLowerCase().includes('rice') ? '🍚' : '🍲'}
                      </div>
                      <div>
              ...
              <div className="text-center py-12 text-muted-foreground text-sm border-2 border-dashed border-neutral-100 dark:border-neutral-800 rounded-3xl flex flex-col items-center gap-3">
                <div className="p-4 rounded-full bg-neutral-50 dark:bg-neutral-800"><Utensils className="w-8 h-8 opacity-20" /></div>
        ...
        <Card className="shadow-sm border-none bg-blue-500 text-white rounded-3xl overflow-hidden relative group">
          <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:scale-110 transition-transform duration-700"><Droplets className="w-32 h-32" /></div>
          <CardHeader className="pb-2 relative">
            <CardTitle className="text-xl font-bold flex items-center gap-3">
              <div className="p-2 rounded-2xl bg-white/20"><Droplets className="w-5 h-5 text-white" /></div>
              Hydration
            </CardTitle>
          </CardHeader>
          <CardContent className="text-center space-y-8 relative pt-4">
            ...
            <div className="flex flex-col gap-2 px-4">
              <Button variant="secondary" className="w-full rounded-2xl font-bold text-blue-600 hover:bg-white transition-colors h-12" onClick={() => handleAddWater(250)}>
                +250ml <span className="ml-2 text-[10px] opacity-60 font-normal">(Glass)</span>
              </Button>
              <Button variant="secondary" className="w-full rounded-2xl font-bold text-blue-600 bg-white/10 border border-white/20 text-white hover:bg-white/20 transition-colors h-12" onClick={() => handleAddWater(500)}>
      ...
      <Card className="shadow-sm border-none bg-orange-50/50 dark:bg-orange-950/10">
        <CardHeader className="pb-2">
          <CardTitle className="text-lg flex items-center gap-2 text-orange-700 dark:text-orange-400"><Lightbulb className="w-5 h-5" />Recommended Healthy Foods</CardTitle>
      ...
      <Card className="shadow-sm overflow-hidden">
        <CardHeader className="bg-primary/5 border-b border-primary/10">
          <CardTitle className="flex items-center gap-2"><ChefHat className="w-5 h-5 text-primary" />Recipe Recommendations</CardTitle>
        ...
        <CardContent className="pt-6">
          <form onSubmit={onSearchSubmit} className="flex items-center gap-4 mb-8">
            <div className="relative flex-grow">
              ...
              <Input placeholder="Find more recipes (e.g. high protein chicken)..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10 h-11 rounded-full border-primary/20 focus-visible:ring-primary" />
            </div>
            <Button type="submit" disabled={isSearching} className="rounded-full h-11 px-8">{isSearching ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Search'}</Button>
          </form>
          {isSearching && recipes.length === 0 ? <div className="flex justify-center py-12"><Loader2 className="w-8 h-8 animate-spin text-primary" /></div> : (
                ...
                <a key={recipe.id} href={`https://spoonacular.com/recipes/${recipe.title.replace(/\s+/g, '-')}-${recipe.id}`} target="_blank" rel="noopener noreferrer" className="block group">
                  <div className="bg-card rounded-2xl overflow-hidden border border-border/50 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 h-full flex flex-col">
                    <div className="relative w-full h-40">
                      <Image src={recipe.image} alt={recipe.title} fill className="object-cover group-hover:scale-105 transition-transform duration-500" priority={index < 4} />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-4">
                        <span className="text-white text-xs font-medium flex items-center gap-1">View Recipe <Plus className="w-3 h-3" /></span>
                      </div>
                    </div>
                    <div className="p-4 flex-grow flex flex-col justify-between">
                      ...
                      <div className="mt-3 pt-3 border-t border-border/50 flex items-center justify-between">
                        <div className="flex items-center gap-1.5 text-primary">
                          ...
                          <span className="text-xs font-bold">{recipe.nutrition.nutrients.find(n => n.name === 'Calories')?.amount.toFixed(0)} kcal</span>
                        </div>
      ...
      <AddFoodModal isOpen={isAddFoodOpen} onClose={() => setIsAddFoodOpen(false)} onFoodAdded={refreshData} />
    </div>
  );
}
